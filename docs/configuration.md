...
Read only when `[features] authzen` is on. Whether there is a PDP at all is the flag; this table says how the document that describes it — `/.well-known/authzen-configuration`, Authorization API 1.0 §9.2 — is served. The PDP identifier itself is not a key here and never will be: it is the tenant's issuer, which is the identifier the well-known URL is derived from and the one §9.2.3 has a PEP compare against. A second spelling would be a second identity for a tenant that already has one.

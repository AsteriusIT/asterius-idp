...
| `limits.revocation_per_address` | integer | 120 | Requests per window to POST /revoke from one address. Bounds rejected client authentication before further cryptographic work. |
| `limits.revocation_per_client` | integer | 1200 | Successful revocation responses per window for one authenticated client, including unknown tokens whose RFC 7009 response is deliberately indistinguishable. |

...
| A1 | G1, G2 | Registers a client that authenticates with a weak or attacker-chosen shared secret, or with nothing, and so needs only a stolen `client_id` to impersonate | FAPI remains the default and rejects every shared-secret method. A tenant and an administrator must explicitly select the standard OIDC profile before `client_secret_basic` exists; `none`, `client_secret_post` and `client_secret_jwt` remain impossible. The server generates 256 random bits, returns them only once, stores only a SHA-256 digest and compares digests in constant time. Rotation replaces and revocation clears that digest atomically (ADR-0014). | `ast-m9c.1`, `ast-dng9` |
...
| A1, A3a | G1, G3 | Registers `require_pushed_authorization_requests: false` and pushes the authorization request through the front channel instead | Refused for every FAPI client. An explicitly tenant-permitted OIDC client may use the direct path; it still passes the same code-flow, exact redirect, PKCE S256, resource, RAR and grant validators, then is stored behind a short-lived opaque internal request reference before entering the interaction pipeline (ADR-0014). | `ast-m9c.1`, `ast-gxh.1`, `ast-dng9` |
...
| A1 | G1, G2 | Updates itself into another client, into an unsuspended one, or into one holding a resource allow-list or agent profile it was never granted | The `update` statement names only the columns a registration document can express. `registration_access_token_hash`, `resources` (`ast-m9c.14`), the agent profile (`ast-lh3.1`), `software_statement`, `status`, `client_type` and `created_at` are untouched by construction rather than by a check. `client_id` must be present and must equal the one the URL names (RFC 7592 §2.2), and `client_secret`, `client_secret_expires_at` and `registration_access_token` are refused outright — a client that asked to set a credential and got a 200 would hold a false belief about its own | `ast-m9c.5`, `ast-m9c.14` |
...
| A1 | G1 | Obtains a token with no audience at all, by naming no `resource` under a client whose allow-list is empty — a token every non-conforming resource server accepts | There is no audience-less path: `ResourceRegistry::targets` returns `invalid_target` for an empty result and `Audience` is non-empty by construction. The only default set is the client's administrator-owned allow-list, filtered through the registry; a resource server an operator withdraws stops being an audience rather than quietly continuing to receive tokens | `ast-gxh.7`, `ast-a05.3`, `ast-m9c.14` |
...
| A1 | G1 | Uses the endpoint as an amplifier: it verifies a `private_key_jwt` assertion — a public-key signature — before it decides anything | `/revoke` has its own `LimitedEndpoint::Revocation` address and authenticated-client budgets. The guard runs before client authentication, token classification and token signature verification, so a flood is stopped before its expensive work; exhaustion answers 429 with `Retry-After` and cannot consume `/token`'s separate budget. Requests admitted by the guard retain RFC 7009 §2.2's empty 200 for unknown, expired, already-revoked and other-client credentials | `ast-1sk.2`, `ast-p2l.3`, `ast-1sk.7` |
...
| **A1**, **A5** | **G1** | **A group role crosses a tenant or client boundary, or becomes console authority.** | Group-role tables use composite tenant/group and tenant/client/role foreign keys. Effective-role resolution always binds the request tenant and token rendering narrows client roles to the token's client. The port accepts `RoleName`, never the closed built-in `Role` enum, so group administration cannot appoint tenant or deployment administrators. Direct and group sources are unioned and deduplicated with provenance, preventing removal of one source from concealing another. |
...
that client, behind the address and authenticated-client endpoint limits
described above. A client in that position can also enumerate through any of half a
...
- **Residual risk.** The limits are fixed windows in the database, so a client
  ...
  *deduplicated* — two requests for the same scopes appear as two lines in the
  ...
  Device authorization has a separate fixed-window address/client budget; it
  does not add a per-person budget because merely creating a code notifies no
  person. The verification page still carries its own failed-code budget.
...
| Some endpoints are still unlimited. | `ast-p2l.3` covers `/register`, the RFC 7592 configuration endpoint, `/par`, `/token` and UserInfo. Introspection, CIBA and device authorization subsequently joined `LimitedEndpoint`; device authorization (`ast-lh3.11`) has independent address and authenticated-client buckets because every attempt verifies a signature and every success creates an approval row. `/revoke` remains the known gap the table above records. The password-reset flow does not exist, while self-registration and email verification are first-party browser flows with their own controls rather than protocol endpoints in this limiter. Discovery, JWKS and `/logout` are reads of documents or of one session row, bounded by the transport's body and timeout limits rather than by a counter. | `ast-2vk.9`, `ast-p2l.3`, `ast-1sk.1`, `ast-5lw`, `ast-lh3.11` |
...
| FAPI 2.0 Message Signing (JAR/JARM/signed introspection) and the separate HTTP Signatures draft are not implemented. | Out of v1 scope; the attacker model above does not require them for G1–G3 at this profile level. | `ast-s36.1` |
...
| The release image is not reproducible bit-for-bit. | `ast-9mg` closed the provenance half of this: `release.yml` signs each image with cosign keyless (the identity is the workflow file at the tag, so there is no long-lived key to steal) and publishes a CycloneDX SBOM for both the binary and the image, the latter attached as a signed attestation. `docs/deployment/verifying-a-release.md` is the verification procedure. What remains is reproducibility — the `Dockerfile` pins `rust:1.98-trixie` by tag rather than digest and its apt layer resolves at build time, so a third party cannot rebuild the digest and compare. The signature says *this repository built it*, not *you could build it again*. **Untested in anger: the workflow has not yet run on a tag.** | `ast-p2l.7` |
| The image inherits the base image's Debian packages, and their advisories. | Closed for libc and OpenSSL as of 2026-09: the binary is statically linked against musl on `distroless/static`, so the image has no `libc6` and no `libssl` — the `aws-lc-rs` musl build that ADR-0004 once ruled out now works from Debian's `musl-tools` alone (`deploy/README.md`, "Why static musl"). What remains is `ca-certificates`, `tzdata`, `netbase`, `base-files` and `media-types`, none of which is executable code; the image SBOM lists them so an advisory against one is still answerable. | `ast-p2l.7` |

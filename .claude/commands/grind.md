...
4. À la réponse du worker :
   - `STATUT: OK` → depuis le dépôt principal : `git merge --no-ff claude/<id> -m "merge(<id>): <titre>"`, `git push origin main`, `git branch -d claude/<id>`, `git worktree prune`. Puis `bd close <id> --reason "<RESUME du worker>"`.

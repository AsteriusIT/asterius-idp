...
fuzz_target!(|data: &[u8]| {
    ...
    let Ok(issuer) = Issuer::parse(text) else {
        return;
    };
    ...
    assert!(
        ...
        "accepted non-https: {canonical:?}"
    );
    assert!(!canonical.contains('?'), "accepted a query: {canonical:?}");
    assert!(
        !canonical.contains('#'),
        "accepted a fragment: {canonical:?}"
    );
    assert!(
        !canonical.ends_with('/'),
        "not canonical, trailing slash: {canonical:?}"
    );
    assert!(
        !issuer.authority().is_empty(),
        "accepted an empty authority: {canonical:?}"
    );

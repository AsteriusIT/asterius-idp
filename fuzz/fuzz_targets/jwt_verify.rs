...
use asterius_jose::{Policy, SigningKey, VerifyingKey, verify};
...
fuzz_target!(|data: &[u8]| {
    ...
    // Every typ and every algorithm combination a caller might ask for, so the
    // policy checks are exercised rather than only the parser.
    for typ in ["at+jwt", "dpop+jwt", "oauth-authz-req+jwt", "logout+jwt"] {
        for algorithms in [
            SigningAlgorithm::ALL.to_vec(),
            vec![SigningAlgorithm::EdDsa],
            Vec::new(),
        ] {
            let policy = Policy::new(typ, algorithms)
